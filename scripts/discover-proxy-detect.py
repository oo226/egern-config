...
OUT_COLLECTION = MODULES / "skip-proxy-collection.module"
...
def _split_append_values(line: str) -> list[str]:
    if "=" not in line:
        return []
    _, _, rhs = line.partition("=")
    rhs = re.sub(r"%APPEND%\s*", "", rhs, flags=re.I).strip()
    return [p.strip() for p in rhs.split(",") if p.strip()]


def merge_general_snippets(entries: list[dict]) -> tuple[str, str]:
    """Return (proxy-detect-extra text, skip-proxy-collection text)."""
    skip_vals: list[str] = []
    realip_vals: list[str] = []
    rules: list[str] = []
    sources: list[str] = []
    for entry in entries:
        ...
        try:
            ...
            continue
        sources.append(entry.get("id") or url)
        section = None
        for line in text.splitlines():
            ...
            if section == "general":
                ...
                if low.startswith("skip-proxy"):
                    skip_vals.extend(_split_append_values(stripped))
                elif low.startswith("always-real-ip"):
                    realip_vals.extend(_split_append_values(stripped))
            elif section == "rule":
                rules.append(stripped)

    def uniq_keep(vals: list[str]) -> list[str]:
        out: list[str] = []
        seen: set[str] = set()
        for v in vals:
            key = v.lower()
    ...
    def uniq_rules(lines: list[str]) -> list[str]:
        out: list[str] = []
        seen: set[str] = set()
        for line in lines:
    ...
    skip_u = uniq_keep(skip_vals)
    real_u = uniq_keep(realip_vals)
    rules_u = uniq_rules(rules)
    if not (skip_u or real_u or rules_u):
        return "", ""

    src_note = "、".join(dict.fromkeys(sources))
    # General skip-proxy / always-real-ip live only in Surge.conf + Egern.yaml.
    # Emitting them here again stacks with modules and trips iOS truncation / memory WARN.
    collection_lines = [
        "#!name=跳过代理检测合集",
        "#!desc=【勿单独安装】skip-proxy / always-real-ip 已写入主配置；本文件仅保留补充 Rule（每日扫描）。",
        "#!category=代理检测",
        "#!tag=skip-proxy,always-real-ip,合集",
        f"# sources: {src_note}",
        "# General lists intentionally omitted — see Egern/Profile.yaml bypass_tunnel_proxy / real_ip_domains",
        "# AUTO-GENERATED by scripts/discover-proxy-detect.py — do not edit",
        "",
    ]
    if rules_u:
        collection_lines.append("[Rule]")
        collection_lines.extend(rules_u)
        collection_lines.append("")
    elif skip_u or real_u:
        # Keep a tiny marker so the file is not empty when upstreams only had General
        collection_lines.append("# (no extra Rule; skip/real-ip folded into profile)")
        collection_lines.append("")

    # proxy-detect-extra: same body, kept as unlock merge input alias
    extra_lines = [
        "#!name=代理检测补充",
        "#!desc=同 skip-proxy-collection，供 unlock 合并引用",
        "#!category=代理检测",
        "",
    ] + collection_lines[collection_lines.index("") + 1 :]
...
def main() -> None:
    ...
    extra, collection = merge_general_snippets(
        [e for e in entries if e.get("upstream") and e.get("url")]
    )
    if collection:
        OUT_COLLECTION.write_text(collection, encoding="utf-8")
        print(f"wrote {OUT_COLLECTION.relative_to(ROOT)} ({len(collection.splitlines())} lines)")
        OUT_MODULE.write_text(extra or collection, encoding="utf-8")
        print(f"wrote {OUT_MODULE.relative_to(ROOT)}")
    else:
        print("skip skip-proxy-collection: nothing to merge")
