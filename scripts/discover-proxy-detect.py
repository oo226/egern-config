...
def merge_general_snippets(entries: list[dict]) -> tuple[str, str]:
    ...
    collection_lines = [
        "#!name=跳过代理检测合集",
        "#!desc=mieqq + Sukka Always Real IP + iamsweet 等合并去重（每日扫描更新）。银行/交管等 App 跳过代理检测用这一份即可。",
        "#!category=代理检测",
    ...
    if skip_u or real_u:
        collection_lines.append("[General]")
        if skip_u:
            collection_lines.append("skip-proxy = %APPEND% " + ", ".join(skip_u))
        if real_u:
            collection_lines.append("always-real-ip = %APPEND% " + ", ".join(real_u))
