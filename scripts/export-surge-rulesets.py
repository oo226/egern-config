...
These are **our** comprehensive lists (often larger than lean lazy packs = more
coverage). Surge.conf must only reference raw URLs on oo226/egern-config —
never third-party RULE-SET hosts at runtime.
    ...
    (STAGING / "README.md").write_text(
        "\n".join(
            [
                ...
                "| 后缀 | Surge 用法 | 说明 |",
                "| --- | --- | --- |",
                "| `.list` | `RULE-SET,url,策略` | 完整规则（不含 DOMAIN/URL-REGEX） |",
                "| `.domainset` | `DOMAIN-SET,url,策略` | 纯域名；`.` 前缀=后缀；IDN→punycode |",
                "| `.ip.list` | `RULE-SET,url,策略,no-resolve` | 仅 IP/ASN |",
                "",
                "生成：`python3 scripts/export-surge-rulesets.py`",
                "主配置：`surge/Surge.conf`（DNS 保持精简；勿把大表挂进 `[Host]`）。",
                "",
            ]
        ),
        encoding="utf-8",
