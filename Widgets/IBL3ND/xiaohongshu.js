...
if (url.includes("/v1/interaction/comment/video/download")) {
  ...
  if (obj?.data?.[0]?.note_list?.length > 0) {
    for (let item of obj.data[0].note_list) {
      ...
      item.hash_tag = [];
      item.topics = [];
      item.desc = cleanTags(item.desc);
      if (item.share_info) {
        item.share_info.content = cleanTags(item.share_info.content);
        item.share_info.wechat_share_desc = cleanTags(item.share_info.wechat_share_desc);
      }
      if (item.qq_mini_program_info) {
        item.qq_mini_program_info.desc = cleanTags(item.qq_mini_program_info.desc);
      }
    }
  }
