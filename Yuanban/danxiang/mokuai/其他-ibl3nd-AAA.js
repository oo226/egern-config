/*
原始来源：https://raw.githubusercontent.com/RuCu6/Loon/main/Scripts/weibo.js
已从 Loon/Surge 的 $request/$response/$done 回调式 API
改写为 Egern 的 export default async function(ctx) API，内部去广告逻辑未做任何改动。
*/
...
export default async function (ctx) {
  ...
  if (url.includes("/interface/sdk/sdkad.php")) {
    ...
    return { body: JSON.stringify(obj) + "OK" };
  } else {
    let obj;
    try {
      obj = JSON.parse(body);
              ...
              for (let ii of item.items) {
                if (ii?.data) {
                  ...
                  if (ii?.data?.comment_bubble) {
                    delete ii.data.comment_bubble;
                  }
                  if (ii?.data?.comment_bullet_screens_message) {
                    delete ii.data.comment_bullet_screens_message;
                  }
                  if (ii?.data?.hot_icon) {
                    delete ii.data.hot_icon;
                  }
                  if (ii?.data?.vip_button) {
                    delete ii.data.vip_button;
                  }
                }
                newII.push(ii);
              }
